                ...
                if(this.formType == "insert"){
                    if(this.receive.date_receipt==""){
                        ...
                        Receive.save({},data).then((response) => {
                            Materialize.toast('<strong><i class="material-icons left">done</i>Recebimento cadastrado com sucesso</strong>'
                    ...
                    Receive.update({id: this.receive.id},data).then((response) => {
                        Materialize.toast('<strong><i class="material-icons left">done</i>Recebimento alterado com sucesso</strong>'
                            ,4000,'green lighten-3');
                        this.$router.go({name: 'bill-receive.list'});
                    });
                }
